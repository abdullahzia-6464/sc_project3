...
from config import COMMUNICATION_RANGE_KM, SATELLITE_PORTS, GROUND_CONTROL_COORDS
...
def get_all_positions():
    ...
    all_positions = [
        {"latitude": positions["ground_control"]["latitude"], "longitude": positions["ground_control"]["longitude"],
         "type": "ground_control"}]
    all_positions += [
        {"latitude": sat["latitude"], "longitude": sat["longitude"], "type": "satellite", "port": sat["port"]} for sat
        in positions["satellites"]]
    if positions["ship"]:
        all_positions.append(
            {"latitude": positions["ship"]["latitude"], "longitude": positions["ship"]["longitude"], "type": "ship"})
